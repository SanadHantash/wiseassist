...
Dashboard.createcourse = async (title,detail,description,trainer,start_time,end_time,category_id,imageUrl,audince_id,site) => {
    ...
    Dashboard.allcourses = async (page, pageSize, searchTerm, categoryFilter, isPaidFilter) => {
      try {
        ...
        if (searchTerm) {
          queryString += ` AND (LOWER(courses.title) LIKE LOWER($${params.length + 1}) OR LOWER(courses.description) LIKE LOWER($${params.length + 1}))`;
          params.push(`%${searchTerm}%`);
        }
    
        if (categoryFilter) {
          queryString += ` AND LOWER(categories.category) = LOWER($${params.length + 1})`;
        ...
        queryString += ` ORDER BY courses.id LIMIT $${params.length + 1} OFFSET $${params.length + 2};`;
    ...
    Dashboard.allworkshops = async (page, pageSize, searchTerm, categoryFilter, isPaidFilter) => {
      try {
        const offset = (page - 1) * pageSize;
        let queryString = `
          SELECT 
  ...
  techtips.is_deleted = false LIMIT $1 OFFSET $2;
    ...
    Dashboard.deletetechtip = async (techId) => {
      try {
        ...
        const result = await db.query('UPDATE techtips SET is_deleted = TRUE  WHERE id = $1', [techId]);
        return result.rows;
      } catch (err) {
        throw err;
      }
    ...
    Dashboard.allquestions = async (page,pageSize) => {
      try {
        const offset = (page - 1) * pageSize;
        const result = await db.query('SELECT faq.id,faq.question,users.user_name from faq  inner join  users on users.id = faq.user_id where faq.is_deletedq = false LIMIT $1 OFFSET $2;',[pageSize, offset]);
       return  result.rows
      } catch (err) {
        throw err;
      }
...
Dashboard.allusers = async (page, pageSize, searchTerm, roleFilter) => {
  try {
    const offset = (page - 1) * pageSize;
    let queryString =
      `SELECT users.id, users.first_name, users.last_name, users.user_name, users.email, users.is_deleted, roles.role FROM users INNER JOIN roles ON roles.id = users.role_id WHERE roles.role != 'admin'`;

    const params = [];

    if (searchTerm) {
      queryString += ` AND LOWER(users.user_name) LIKE LOWER($${params.length + 1})`;
      params.push(`%${searchTerm}%`);
    }
    ...
    queryString += ` ORDER BY users.id LIMIT $${params.length + 1} OFFSET $${params.length + 2};`;
...
Dashboard.attendances = async (courseID) => {
  try{
    const result = await db.query('select course_attendances.id ,users.user_name,users.email,users.phonenumber,roles.role from course_attendances  inner join users on users.id = course_attendances.user_id inner join roles on roles.id = users.role_id where course_id = $1',[courseID]);
    return result.rows
  }catch (err) {
    throw err;
  }
...
Dashboard.countattendances = async (courseID)=>{
  try{
    const result = await db.query('select count(id) from course_attendances where course_id =$1',[courseID])
    return result.rows
  }catch (err) {
    throw err;
  }
...
Dashboard.videoviewers = async (lessonID) => {
  try{
    const result = await db.query('select watched_videos.id ,users.user_name,users.email,users.phonenumber,roles.role from watched_videos  inner join users on users.id = watched_videos.user_id inner join roles on roles.id = users.role_id where lesson_id = $1',[lessonID]);
    return result.rows
  }catch (err) {
    throw err;
  }
...
Dashboard.countvideoviewers = async (lessonID)=>{
  try{
    const result = await db.query('select count(id) from watched_videos where lesson_id =$1',[lessonID])
    return result.rows
  }catch (err) {
    throw err;
  }
...
Dashboard.mostenrolledcourse = async()=>{
  try{
    const result = await db.query('SELECT course_id,courses.title FROM course_attendances inner join courses on courses.id = course_attendances.course_id where courses.is_deleted = false and (courses.category_id = 1 or courses.category_id = 2)GROUP BY course_id,courses.title ORDER BY COUNT(course_id) DESC LIMIT 1');
    return result.rows
  }catch (err) {
    throw err;
  }
